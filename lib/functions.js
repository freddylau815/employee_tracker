...
async function updateEmployeeRole() {
  try {
    ...
    const roleTitleQuery = `SELECT id, title FROM roles;`
    const[roleTitles] = await db.promise().query(roleTitleQuery) 
    ...
    const selectRole = [
        {
            ...
            choices: roleTitles.map(({ id, title }) => ({
                name: title,
                value: id
    ...
    const { selectedRole } = await inquirer.prompt(selectRole);

    const query = `
        UPDATE employees SET role_id = (?) WHERE id = (?);
      `;

    const employeeParams = [selectedRole, employee_id]
