...
async function updateEmployeeRole() {
  try {
    // Get names 
    const employeeNameQuery = `SELECT CONCAT (first_name, ' ', last_name) AS employee_name FROM employees;`
    ...
    const selectEmployee = [
      {
        ...
        choices: employeeNames.map(({ employee_name }) => employee_name)
      }
    ];

    const { selectedEmployee } = await inquirer.prompt(selectEmployee);
...
//--------------
    ...
    `
    const [employee] = await db.promise().query(nameQuery, selectedEmployee);
...
//--------------
    ...
    const employeeParams = [roleId, employeeId]
